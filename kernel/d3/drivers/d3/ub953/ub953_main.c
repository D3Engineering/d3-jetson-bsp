...
struct ub953_cfg {
	...
	int hs_clk_div;
	int div_n_val;
	// If the GPIO controller functionality is not used by the imager, then the
	// UB953 can setup the GPIO pins using the functionality below.
	int gpio_rmten;
	int gpio_out_src;
	int gpio_out_en;
	int gpio_in_en;
...
struct ub953
{
	...
	/**
	 * GPIO controller instance loaded by this driver. Once the work around
	 * approach of having the UB953 load the GPIO controller is no longer
 ...
 * @param gpio_device The platform device representing the GPIO controller.
 * @param gpio_node of device node
 *
 * @return 0 on success.
 */
static int ub953_gpio_create(struct ub953 *self)
{
	int err = 0;

	struct platform_device_info gpio_dev_info = {
		.name = "ub953-gpio",
		.parent = self->dev,
		.id = self->client->addr
	};

	// Register the device, will run the probe logic
	self->gpio = platform_device_register_full(&gpio_dev_info);
	if(IS_ERR(self->gpio)) {
		dev_dbg(self->dev, "Failed to run probe of GPIO");
		self->gpio = NULL;
	}
	return err;
}
...
static int ub953_init(struct ub953 *self, const struct ub953_cfg *cfg)
{
	...
	// If the GPIO controller functionality is not used, then the
	// GPIO settings will be set up below.
	if (cfg->gpio_rmten >= 0 && cfg->gpio_out_src >= 0) {
		dev_dbg(self->dev, "Setting logical output of GPIO's: %#x",
	...
	if (cfg->gpio_out_en >= 0 && cfg->gpio_in_en >= 0) {
		dev_dbg(self->dev, "enabling input and output of GPIO's: %#x",
...
static int ub953_cfg_load(struct ub953 *self, struct ub953_cfg *out)
{
	...
	// In systems that make use of the UB953 gpio controller, the GPIO values
	// listed below will not be present
		       ...
		       const struct i2c_device_id *id)
{
	...
	// The 953 GPIO controller should NOT be initialized in the following
	// situations:
	// 1. The 953's GPIOs are controlled directly by this driver to reset
	//    the sensor
	// 2. Systems that wait-for-configuration due to an MCU which controls a
	//    GPIO line shared with the 953
	//
	// Creating the GPIO controller clears the 953's GPIO registers. In both
	// cases, the sensor could be held in reset or otherwise cause
	// unpredictable behavior.
	if((self->cfg.gpio_out_en == -1 && self->cfg.gpio_out_src == -1) &&
...
/**
 * Setup the frame sync GPIO for remote control output which will allow the
 * system to send a frame sync signal to the imager.
 *
...
int ub953_set_frame_sync_enable(struct device *dev, bool enabled)
{
	...
	if (self->cfg.fsync_gpio >= 0) {
		...
		// Enable the GPIO as a remote GPIO
		TRY(err, regmap_update_bits(self->map, UB953_REG_LOCAL_GPIO_DATA,
			mask, val));
		// Enable the GPIO as an output
		TRY(err, regmap_update_bits(self->map, UB953_REG_GPIO_INPUT_CTRL,
			mask, val));
	} else {
		dev_dbg(self->dev, "No fsync gpio found");
	}
	return err;
}
