...
#define DIGITAL_GAIN_FIXED_BITS	(8)
...
{
	u8 analog_gain_exponent = 0;
	u64 digital_gain_fixed = 0;
	/* TODO: It is documentation that high conversion gain mode is
	 * 2.57. We should make use of high conversion gain mode, when
	 * appropriated, but the logic is not here yet. */
	u8 is_high_conversion_gain = 0;
	u16 raw_gain = 0;
	u16 raw_dig_gain = 0;
	u8 raw_dig_gain_l = 0;
	u8 raw_dig_gain_h = 0;
	...
	if (val >= 8 * FIXED_POINT_SCALING_FACTOR) {
		analog_gain_exponent = 3;
		/* is_high_conversion_gain = 1; */
	} else if (val >= 4 * FIXED_POINT_SCALING_FACTOR) {
		analog_gain_exponent = 2;
		/* is_high_conversion_gain = 1; */
	} else if (val > 2 * FIXED_POINT_SCALING_FACTOR) {
		analog_gain_exponent = 1;
		/* is_high_conversion_gain = 1; */
	} else if (val >= 1 * FIXED_POINT_SCALING_FACTOR) {
		analog_gain_exponent = 0;
		/* is_high_conversion_gain = 1; */
	} else {
		analog_gain_exponent = 0;
		/* is_high_conversion_gain = 0; */
	}
	...
	// gain of "val"
	digital_gain_fixed = val / (1 << analog_gain_exponent);

	dev_dbg_ratelimited(self->dev,
			    "Changing gain: %lld == %llu",
			    val,
			    digital_gain_fixed * (1 << analog_gain_exponent));

	digital_gain_fixed =
		(digital_gain_fixed >>
		 (FIXED_POINT_BITS - DIGITAL_GAIN_FIXED_BITS));
	raw_dig_gain = digital_gain_fixed & 0x3FFF;
	raw_dig_gain_l = raw_dig_gain & 0xFF;
	raw_dig_gain_h = (raw_dig_gain >> 8) & 0xFF;

	/* Be extremely cautious that no unintended bits are set. */
	is_high_conversion_gain &= 1;
	analog_gain_exponent &= 3;
	raw_gain = (is_high_conversion_gain << 7)
		| (is_high_conversion_gain << 6)
		| (analog_gain_exponent << 4)
		| (analog_gain_exponent << 2)
		| analog_gain_exponent;
	...
	TRY(err, ov10640_group_reg_write(self, OV10640_DIG_S_GAIN_L,
					 raw_dig_gain_l));
	TRY(err, ov10640_group_reg_write(self, OV10640_DIG_S_GAIN_H,
					 raw_dig_gain_h));
	TRY(err, ov10640_group_reg_write(self, OV10640_DIG_VS_GAIN_L,
					 raw_dig_gain_l));
	TRY(err, ov10640_group_reg_write(self, OV10640_DIG_VS_GAIN_H,
					 raw_dig_gain_h));
...
int ov10640_ctrls_init(struct ov10640 *self)
{
	...
	if (ctrl == NULL) {
		dev_err(self->dev, "Error initializing hflip control\n");
		err = -EINVAL;
		goto error;
	}
	...
	if (ctrl == NULL) {
		dev_err(self->dev, "Error initializing vflip control\n");
		err = -EINVAL;
		goto error;
	}
	self->ctrls[ctrl_index++] = ctrl;
