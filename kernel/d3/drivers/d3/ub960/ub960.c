...
static int ub960_gpios_get(struct ub960 *self)
{
	...
	ub960_gpio_get(self, "pwdn-gpios", &self->pwdn_gpio);
	ub960_gpio_get(self, "reset-gpios", &self->reset_gpio);

	if (self->pwdn_gpio) {
		TRY(err, ub960_gpio_request(self, self->pwdn_gpio,
					    "cam_pwdn_gpio"));
		TRY(err, ub960_gpio_export(self, self->pwdn_gpio,
					   "cam_pwdn_gpio"));
		TRY(err, ub960_gpio_direction_output(self, self->pwdn_gpio,
						     "pwdn_gpio", 0));
	}

	if (self->reset_gpio) {
		TRY(err, ub960_gpio_request(self, self->reset_gpio,
					    "cam_reset_gpio"));
		TRY(err, ub960_gpio_export(self, self->reset_gpio,
					   "cam_reset_gpio"));
		TRY(err, ub960_gpio_direction_output(self, self->reset_gpio,
						     "reset_gpio", 0));
	}
...
static int ub960_reset(struct ub960 *self)
{
	...
	if (self->reset_gpio) {
		ub960_gpio_set(self->reset_gpio, false);
		usleep_range(3 * 1000, 4 * 1000);
		ub960_gpio_set(self->reset_gpio, true);
		usleep_range(3 * 1000, 4 * 1000);
	} else {
...
{
	int err = 0;
	TRY(err, regmap_write(self->map, UB960_REG_SLAVE_ID0 + id, addr));
	return 0;
}
...
static int ub960_chan_cfg(struct ub960 *self, u8 id)
{
	...
	struct reg_sequence seq[] = {
		...
		// TODO: Currently, performing this register write causes I2C communication
		// to stop working reliably.  So, for now, I am manually writing them using
		// i2cset after startup.
		{UB960_REG_BC_GPIO_CTL0,    0x8a}, // Route FrameSync to BC_GPIO0
	};
